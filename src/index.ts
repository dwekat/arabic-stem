...
export default class Stemmer {
  affixCleaner: AffixCleaner | undefined;

  /**
   *
   * @param {string} token
   * @return {{stem: string[], normalized: string}}
   */
  stem(token: string): { stem: string[]; normalized: string } {
    token = token.trim();
    token = token.replace(re_short_vowels, '');

    if (stop_words.includes(token) || token.length < 3) {
      return { stem: [], normalized: token };
    }

    token = this.preNormalize(token);

    this.affixCleaner = new AffixCleaner(token);
    token = this.affixCleaner.remove(4, Priority.prefix, true);
    token = this.affixCleaner.remove(3, Priority.prefix, true);
    token = this.affixCleaner.remove(2, Priority.prefix, true);

    let matches = this.getMatches(token, Priority.suffix);
    matches = matches.concat(this.getMatches(token, Priority.prefix));
   ...
   *
   * @param {string} token
   * @param {Priority} removeFirst
   * @param {boolean} inRecursion
   * @return {string[]}
   */
  getMatches(token: string, removeFirst: Priority = Priority.suffix, inRecursion = false): string[] {
    //TODO: Find a way to make sure that affixCleaner is always defined
    ...
    while (len > 3) {
      ...
      token = this.affixCleaner.remove(1, Priority.suffix, false);
